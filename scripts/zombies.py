...
class Zombie:
    ...
    def draw_on_image(self, photo: np.ndarray, aim, debug=False):
        black = [0, 0, 0]
        ...
        cv.circle(photo,
                  ...
                  math.ceil(0.2 * self.size), black, -1)
        cv.circle(photo,
                  [relative_location[0] - math.ceil(5 * self.size), relative_location[1] - math.ceil(10 * self.size)],
                  math.ceil(0.2 * self.size), black, -1)
        # body
        ...
        if self.frames_lived > 1400:
            eyebrow1_point1 = [relative_location[0] - math.ceil(6 * self.size),
                               relative_location[1] - math.ceil(12.5 * self.size)]
            eyebrow1_point2 = [relative_location[0] - math.ceil(3.5 * self.size),
                               relative_location[1] - math.ceil(11.5 * self.size)]
            eyebrow2_point1 = [relative_location[0] - math.ceil(6 * self.size),
                               relative_location[1] - math.ceil(12.5 * self.size)]
            eyebrow2_point2 = [relative_location[0] - math.ceil(3.5 * self.size),
                               relative_location[1] - math.ceil(11.5 * self.size)]

            cv.line(photo, eyebrow1_point1, eyebrow1_point2, black, math.ceil(0.5 * self.size))
            cv.line(photo, eyebrow2_point1, eyebrow2_point2, black, math.ceil(0.5 * self.size))

            cv.line(photo, [relative_location[0] + math.ceil(6 * self.size),
                            relative_location[1] - math.ceil(12.5 * self.size)],
                    [relative_location[0] + math.ceil(3.5 * self.size),
                     relative_location[1] - math.ceil(11.5 * self.size)], black,
                    math.ceil(0.5 * self.size))

            cv.circle(photo, [relative_location[0], relative_location[1] - math.ceil(5 * self.size)],
                      math.ceil(2.5 * self.size), black, -1)
